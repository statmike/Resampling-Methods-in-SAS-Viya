...
proc cas;
	...
	resample.doubleBootstrap / intable='sample' B=50 D=10 seed=12345 Bpct=1 Dpct=1 case='unique_case' strata='none';
run;
...
proc cas;
  ...
  fedsql.execDirect / query='create table sample_dbs_pred {options replace=TRUE} as select distinct caseID, dbsID, bsID, bag, pred, resid from sample_dbs_pred';
  ...
  percentile / table = {name='sample_bs_pred', groupBy="CASEID", vars={"Pred"}},
    ...
    values = {2.5, 50, 97.5};
  percentile / table = {name='sample_dbs_pred', groupBy="CASEID", vars={"Pred"}},
    casOut = {name='sample_dbs_pred_perc', replace=TRUE},
    values = {2.5, 50, 97.5};
  ...
  fedsql.execDirect / query="create table sample_bs_pred_perc {options replace=true} as
                                select * from
                                    ...
                                    using (CASEID)";
  fedsql.execDirect / query="create table sample_dbs_pred_perc {options replace=true} as
                                select * from
                                    (select CASEID, _Value_ as DBS_LowerCL from sample_dbs_pred_perc where (_pctl_=2.5 and _column_='Pred')) a
                                    join
                                    (select CASEID, _Value_ as DBS_Estimate from sample_dbs_pred_perc where (_pctl_=50 and _column_='Pred')) b
                                    using (CASEID)
                                    join
                                    (select CASEID, _Value_ as DBS_UpperCL from sample_dbs_pred_perc where (_pctl_=97.5 and _column_='Pred')) c
                                    using (CASEID)";
  ...
  fedsql.execDirect / query='create table sample_bs_pred_perc {options replace=true} as
  								select * from
  									...
  									using(CASEID)
										join
										(select * from sample_dbs_pred_perc) c
										using(CASEID)';
  ...
  datastep.runcode result=t / code='data sample_bs_pred_perc; set sample_bs_pred_perc;
                                          ...
                                          resid_DBS_LowerCL=DBS_LowerCL-MSRP;
                                          resid_DBS=DBS_Estimate-MSRP;
                                          resid_DBS_UpperCL=DBS_UpperCL-MSRP;
                                    run;';
run;
...
proc sgplot data=sample_bs_pred_perc;
  ...
  scatter y=caseid x=MSRP / legendlabel='Actual Value';
	...
	refline 0 / axis=x;
run;
title "doubleBootstrap Confidence Intervals for Predictions";
proc sgplot data=sample_bs_pred_perc;
	scatter y=caseid x=DBS_Estimate / xerrorlower=DBS_LowerCL xerrorupper=DBS_UpperCL markerattrs=(symbol=circle size=9 color=red) legendlabel='Prediction';
	scatter y=caseid x=resid_DBS / xerrorlower=resid_DBS_LowerCL xerrorupper=resid_DBS_UpperCL markerattrs=(symbol=circlefilled size=6 color=green) legendlabel='Residual';
  scatter y=caseid x=MSRP / legendlabel='Actual Value';
	yaxis display=(nolabel);
	refline 0 / axis=x;
run;
...
proc sgplot data=sample_bs_pred_perc;
  scatter x=caseid y=resid_BS / markerattrs=(symbol=circlefilled size=6 color=red) legendlabel='Residual';
	scatter x=caseid y=resid_DBS / markerattrs=(symbol=circlefilled size=6 color=blue) legendlabel='Residual';
  refline 0 / axis=y;
run;
...
proc sgplot data=sample_bs_pred_perc;
  scatter x=caseid y=resid_BS / yerrorlower=resid_BS_LowerCL yerrorupper=resid_BS_UpperCL markerattrs=(symbol=circlefilled size=6 color=red) legendlabel='Residual';
	scatter x=caseid y=resid_DBS / yerrorlower=resid_DBS_LowerCL yerrorupper=resid_DBS_UpperCL markerattrs=(symbol=circlefilled size=6 color=blue) legendlabel='Residual';
  refline 0 / axis=y;
run;
title "Residual Plot with Bootstrap Intervals";
