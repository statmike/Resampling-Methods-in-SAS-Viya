/*
An example of using the predicted values from each residual bootstrap resample, including the out-of-bag rows,
to create confidence intervals for the predicted value as percentiles from the fitted residual bootstrap resamples.
Based on example 7
*/
...
proc cas;
  ...
  fedsql.execDirect / query='create table sample_bs_pred_perc {options replace=true} as
  								select * from
  									...
  									left outer join
  									(select CASEID, MSRP from sample_pred) b
  									using(CASEID)';
...
title "Residual Bootstrap Confidence Intervals for Predictions";
